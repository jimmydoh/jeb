...
class Simon(GameMode):
    """Classic Simon Memory Game Mode."""
    def __init__(self, core, speed_factor=0.5, timeout_ms=3000):
    ...
    async def run(self):
        ...
        await self.core.display.update_status("SIMON", "FOLLOW THE LIGHTS")
        ...
        while True:
            ...
            for val in sequence:
                ...
                self.core.matrix.draw_quadrant(val, self.colors[val])
                await self.core.leds.flash_led(
                    val,
                    self.colors[val],
                    brightness=0.8,
                    priority=3,
            ...
            # 3. User Input Phase
            await self.core.display.update_status(f"LEVEL {self.level}", "YOUR TURN")
                ...
                while user_input == -1:
                    ...
                    for i in range(4):
                        if self.core.hid.is_pressed(i):
                            ...
                            await self.core.matrix.draw_quadrant(i, self.colors[i])
                            await self.core.leds.solid_led(
                                i,
                                self.colors[i],
                                brightness=0.8,
                                priority=3
            ...
            await self.core.leds.start_rainbow(speed=0.1)
            await self.core.audio.play(
                "audio/simon/round_win.wav",
                self.core.audio.CH_SFX,
                level=0.6
            )
            round_duration = ticks_diff(ticks_ms(), input_phase_start)
            round_score = 10 * self.level
            await self.core.display.update_status("ROUND COMPLETE!", f"TIME: {round_duration}ms")
