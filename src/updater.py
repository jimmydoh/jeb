...
class Updater:
    ...
    def __init__(self, config, wifi_manager, sd_mounted=False):
        """
        ...
        if wifi_manager is None:
            raise UpdaterError("Wi-FiManager instance is required for OTA updates")

        if not self.update_url:
            raise UpdaterError(
                "Missing required config: update_url"
            )
